	...
	function Tag({value, number}){
		return (
			<div className='row tag' data-number={number} style={{marginBlockEnd:'20px'}}>
				...
				<div className='col-md-2'><button className='btn btn-outline-danger w-100' type='button' onClick={(e) => { removeTag(e.target.closest('.tag'));}}>Delete</button></div>
			</div>
		)
	}
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				<div className="mb-3">
					...
					<input
						...
						onInput={(e) => {changeInput(e.target)}}
						/>
				...
				<div className="mb-3">
					...
					<div className='row tag'>
						...
						<div className='col-md-2'><button className='btn btn-outline-primary w-100' type='button' onClick={(e) => {addTag(e.target);}}>Add tag</button></div>
					</div>
				</div>
				<div className='row'>
					<div className='col-md-5'>
						<button className="w-100 btn btn-lg btn-primary" type='submit' onClick={(e) => {console.log(e); alert('send');}}>Send</button>
					</div>
				</div>
			</form>
		</div>
